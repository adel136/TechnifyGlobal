...
function initCarousel( options ) {
    ...
    this.slidesExt   = document.querySelector( '.slides2' ).children;
		this.slidesN     = this.slides.length;
		this.pagination  = this.node.querySelector( '.pagination' );
    this.paginationExt  = document.querySelector( '.pagination2' );
		this.pagTransf   = 'translate( -50%, -50% )';
		this.dots        = this.pagination.children;
    this.dotsExt     = this.paginationExt.children;
	...
	CustomCarousel.prototype.addListeners = function () {
		...
		for ( var i = 0; i < this.dotsExt.length; i++ ) {		
			this.dotsExt[i].addEventListener( 'click', function( i ) {
				return function() { slider.setSlide( i ); }
			}( i ));
		}    
	...
	CustomCarousel.prototype.setSlide = function ( slideN ) {
		...
		this.slidesExt[ this.activeN ].classList.remove( 'active' );
    ...
    this.slidesExt[ this.activeN ].classList.toggle( 'active' );	
